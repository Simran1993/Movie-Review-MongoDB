...
@movie_bp.route('/')
def index():
    response = requests.get(f'{MOVIE_API_URL}&s=Star Wars') 
...
def movie_details(movie_id):
    ...
    movie_ratings_summary=current_app.review_model.get_movie_ratings(movie_id)
    return render_template('movie_details.html', movie=movie_data, reviews=reviews,movie_ratings_summary=movie_ratings_summary)
